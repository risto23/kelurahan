...
<head>
    ...
    <title>Kecamatan Panggarangan | Profil Kecamatan</title>
</head>
            ...
            <div class="row">
                <div class="col-md-9 col-sm-12 col-xs-12">
                    <div id="content" class="site-content">
                        <main id="main">
                        <?php
                        $proseskec = mysqli_query($koneksi,"select * from tb_profilkec left join tb_profil ON tb_profilkec.id_login = tb_profil.id_login LIMIT 1");
                        while ($datakec = mysqli_fetch_array($proseskec)) {
                        ?>
                            <article class="post format-standard mb-40">
                                <div class="post-thumbnail">
                                    <img src="admin/news/profil/<?php echo $datakec['img_profil']; ?>" alt="<?php echo $datakec['img_profil']; ?>" />
                                </div><!--/ post-thumbnail -->

                                <header class="entry-header">
                                    <h2 class="entry-title h4">
                                       <?php echo $datakec['judul_profil']; ?>
                                    </h2>
                                    <span class="byline">Posted by 
                                        <?php echo $datakec['nama_lengkap']; ?> - 
                                    </span>
                                    <span class="posted-on">
                                        <?php echo fungsi_tgl($datakec['date_profil']);?>
                                    </span>
                                </header><!-- /header -->
                                <div class="entry-content">
                                    <?php echo $datakec['desc_profil'];?>
                                </div><!-- /entry-content -->
                            </article><!-- /post -->
                        <?php  
                        }
