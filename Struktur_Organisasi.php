...
<head>
    ...
    <title>Kecamatan Panggarangan | Struktur Organisasi</title>
</head>
            ...
            <div class="row">
                <div class="col-md-9 col-sm-12 col-xs-12">
                    <div id="content" class="site-content">
                        <main id="main">
                        <?php
                        $proseskec = mysqli_query($koneksi,"select * from tb_strukturorganisasi left join tb_profil ON tb_strukturorganisasi.id_login = tb_profil.id_login LIMIT 1");
                        while ($datakec = mysqli_fetch_array($proseskec)) {
                        ?>
                            <article class="post format-standard mb-40">
                                <header class="entry-header">
                                    <h2 class="entry-title h4">
                                       <?php echo $datakec['judul_struktur']; ?>
                                    </h2>
                                    <span class="byline">Posted by 
                                        <?php echo $datakec['nama_lengkap']; ?> - 
                                    </span>
                                    <span class="posted-on">
                                        <?php echo fungsi_tgl($datakec['date_struktur']);?>
                                    </span>
                                </header><!-- /header -->
                                <div class="entry-content">
                                     <iframe style="margin-top: 20px;" id="viewer" src="admin/news/pdf/<?php echo $datakec['pdf_struktur'];?>" frameborder="0" scrolling="no" width="100%" height="700"></iframe>
                                </div><!-- /entry-content -->
                            </article><!-- /post -->
                        <?php  
                        }
